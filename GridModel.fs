...
module GridModel =
    ...
    let constructBlank (width: int, height: int) : GridModel =
        { Width = width
          Height = height
          Electrodes = Array2D.create width height { ChemList = [] ; Charge = 0.0}  } 
    ...
    let rec moveChem (pos : GridPosition, dest : GridPosition, chem : Chemical) (grid : GridModel) : GridModel =
        if checkIfNeighbour (pos,dest) then
            setElectrode (pos, (removeChem ((getElectrode pos grid),chem))) grid |> setElectrode (dest,(addChem ((getElectrode dest grid),chem)))
        else 
        match pos,dest with
